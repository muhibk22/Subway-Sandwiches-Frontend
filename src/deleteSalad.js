...
function DeleteSalad() {
  ...
  const handleDelete = async () => {
    setMessage('');
    try {
      const response = await fetch(`http://localhost:5000/api/salad/deletesalad?name=${name}`, {
        method: 'DELETE',
      });
  ...
  return (
    <div className="App">
      ...
      <h1 className="text">Delete Salad Record</h1>
      {!record ? (
