...
function Updatedessert() {
  ...
  const handleSearch = async () => {
    setMessage('');
    try {
      const response = await fetch(`http://localhost:5000/api/dessert/viewdesserts?name=${name}`);
      if (response.ok) {
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/dessert/updatedessert`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <div className="App">
      <h1 className="text">Update Desserts Record</h1>
      {!record ? (
