...
function Updatedrink() {
  ...
  const handleSearch = async () => {
    setMessage('');
    try {
      const response = await fetch(`http://localhost:5000/api/drink/viewdrink?name=${name}`);
      if (response.ok) {
  ...
  const handleFormSubmit = async (event) => {
    ...
    const updatedData = { name, type, price, availability };
  
    try {
      const response = await fetch(`http://localhost:5000/api/drink/updatedrink`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <div className="App">
      <h1 className="text">Update Drinks Record</h1>
      {!record ? (
